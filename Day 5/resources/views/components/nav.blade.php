    ...
    <div class="relative flex h-16 items-center justify-between">
      ...
      <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
        <div class="flex shrink-0 items-center">
          <img src={{ asset('linux.webp') }} alt="Your Company" class="h-8 w-auto" />
        </div>
        <div class="hidden sm:ml-6 sm:block">
          ...
          <div class="flex space-x-4">
            ...
            <a href="{{ route('products.index') }}" aria-current="page" class="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white">home</a>
            <a href="{{ route('products.create') }}"class="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">create</a>
           <form action="{{ route('logout') }}" method="POST">
            <button type="submit" class="text-white cursor-pointer">Logout</button>
           </form>
            @endauth

          {{-- if user is not authenticated --}}
          @guest
          <a href="{{ route('showLogin') }}" class="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">login</a>
          <a href="{{ route('showLogin') }}" class="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">register</a>
          @endguest
          </div>
        </div>
      </div>
