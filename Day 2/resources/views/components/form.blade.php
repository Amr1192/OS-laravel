  ...
  <div class="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
    ...
    <form class="space-y-4" method="POST" action="{{ url('/products') }}">
      <div>
        ...
        <input type="text" name="name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all" placeholder="John Doe" />
      </div>
       {{-- @error('name')
    ...
    @enderror --}}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {{-- <div>
          <label class="block text-sm font-medium text-gray-600 mb-1">Confirm</label>
          <input type="password" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all" placeholder="••••••••" />
        </div> --}}
      </div>

      {{-- <div class="flex items-start text-sm">
        <input type="checkbox" class="mt-1 mr-2 rounded border-gray-300 text-indigo-600">
        <span class="text-gray-500">I agree to the <a href="#" class="text-indigo-600 hover:underline">Terms & Conditions</a></span>
      </div> --}}
